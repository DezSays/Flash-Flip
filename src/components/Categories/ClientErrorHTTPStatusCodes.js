...
const ClientErrorHTTPStatusCodes = () => {
  ...
  const getData = async () => {
    ...
    const lvl1 = data.client_error_http_status_codes.level[0].one[`${idx1}`];
    ...
    let idx2 = getRandomInt(0, 4);

    const lvl2 = data.client_error_http_status_codes.level[1].two[`${idx2}`];
    ...
    let idx3 = getRandomInt(0, 8);

    const lvl3 = data.client_error_http_status_codes.level[2].three[`${idx3}`];
    ...
    let idx4 = getRandomInt(0, 7);

    const lvl4 = data.client_error_http_status_codes.level[3].four[`${idx4}`];
    ...
    const lvl5 = data.client_error_http_status_codes.level[4].five[`${idx5}`];
  ...
  if (display === 0) {
    return (
      <>
        <Stack gap={2}>
          <button id="category-title">Client Error HTTP Status Codes</button>
          <button
...
export default ClientErrorHTTPStatusCodes;
