...
const GitBasics = () => {
  ...
  const getData = async () => {
    ...
    const lvl1 = data.git_basics.level[0].one[`${idx1}`];
    ...
    const lvl2 = data.git_basics.level[1].two[`${idx2}`];
    ...
    const lvl3 = data.git_basics.level[2].three[`${idx3}`];
    ...
    const lvl4 = data.git_basics.level[3].four[`${idx4}`];
    ...
    const lvl5 = data.git_basics.level[4].five[`${idx5}`];
  ...
  if (display === 0) {
    return (
      <>
        <Stack gap={2}>
          <button id="category-title">Git Basics</button>
          <button
