...
const Redux = () => {
  ...
  const getData = async () => {
    ...
    const lvl1 = data.redux.level[0].one[`${idx1}`];
    ...
    const lvl2 = data.redux.level[1].two[`${idx2}`];
    ...
    const lvl3 = data.redux.level[2].three[`${idx3}`];
    ...
    let idx4 = getRandomInt(0, 3);

    const lvl4 = data.redux.level[3].four[`${idx4}`];
    ...
    const lvl5 = data.redux.level[4].five[`${idx5}`];
  ...
  if (display === 0) {
    return (
      <>
        <Stack gap={2}>
          <button id="category-title">Redux</button>
          <button
