...
const SQLKeywords = () => {
  ...
  const getData = async () => {
    ...
    const lvl1 = data.sql_keywords.level[0].one[`${idx1}`];
    ...
    let idx2 = getRandomInt(0, 12);

    const lvl2 = data.sql_keywords.level[1].two[`${idx2}`];
    ...
    let idx3 = getRandomInt(0, 22);

    const lvl3 = data.sql_keywords.level[2].three[`${idx3}`];
    ...
    let idx4 = getRandomInt(0, 16);

    const lvl4 = data.sql_keywords.level[3].four[`${idx4}`];
    ...
    const lvl5 = data.sql_keywords.level[4].five[`${idx5}`];
  ...
  if (display === 0) {
    return (
      <>
        <Stack gap={2}>
          <button id="category-title">SQL Keywords</button>
          <button
