...
const HTMLBasics = () => {
  ...
  const getData = async () => {
    ...
    let idx1 = getRandomInt(0, 6);

    const lvl1 = data.html_basics.level[0].one[`${idx1}`];
    ...
    let idx2 = getRandomInt(0, 21);

    const lvl2 = data.html_basics.level[1].two[`${idx2}`];
    ...
    let idx3 = getRandomInt(0, 21);

    const lvl3 = data.html_basics.level[2].three[`${idx3}`];
    ...
    let idx4 = getRandomInt(0, 11);

    const lvl4 = data.html_basics.level[3].four[`${idx4}`];
    ...
    let idx5 = getRandomInt(0, 14);

    const lvl5 = data.html_basics.level[4].five[`${idx5}`];
  ...
  if (display === 0) {
    return (
      <>
        <Stack gap={2}>
          <button id="category-title">HTML Basics</button>
          <button
