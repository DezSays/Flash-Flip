...
const ReactJS = () => {
  ...
  const getData = async () => {
    ...
    const lvl1 = data.react.level[0].one[`${idx1}`];
    ...
    const lvl2 = data.react.level[1].two[`${idx2}`];
    ...
    const lvl3 = data.react.level[2].three[`${idx3}`];
    ...
    const lvl4 = data.react.level[3].four[`${idx4}`];
    ...
    const lvl5 = data.react.level[4].five[`${idx5}`];
  ...
  if (display === 0) {
    return (
      <>
        <Stack gap={2}>
          <button id="category-title">React</button>
          <button
