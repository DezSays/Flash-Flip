...
const Vocab = () => {
  ...
  const getData = async () => {
    ...
    let idx1 = getRandomInt(0, 7);

    const lvl1 = data.vocab.level[0].one[`${idx1}`];
    ...
    let idx2 = getRandomInt(0, 15);

    const lvl2 = data.vocab.level[1].two[`${idx2}`];
    ...
    let idx3 = getRandomInt(0, 27);

    const lvl3 = data.vocab.level[2].three[`${idx3}`];
    ...
    let idx4 = getRandomInt(0, 18);

    const lvl4 = data.vocab.level[3].four[`${idx4}`];
    ...
    const lvl5 = data.vocab.level[4].five[`${idx5}`];
  ...
  if (display === 0) {
    return (
      <>
        <Stack gap={2}>
          <button id="category-title">Vocab</button>
          <button
